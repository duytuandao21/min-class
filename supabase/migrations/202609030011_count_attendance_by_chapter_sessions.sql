...
begin
  ...
  -- One started Chapter Session is one class meeting, regardless of how many
  -- Lessons that Chapter contains. Starting the Chapter again creates another
  -- class meeting and therefore another attendance opportunity.
  select count(*)::integer
  into v_total_class_meetings
  from public.rooms
  where rooms.course_section_id = v_course_section.id
    and rooms.chapter_id is not null
  ...
  attendance_totals as (
    select
      ...
      count(distinct session_attendance.session_id)::integer as attended_class_meeting_count
