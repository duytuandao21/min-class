...
export async function fetchStudentLessonSnapshot(input: string): Promise<StudentLessonSnapshot> {
  const roomId = roomIdSchema.parse(input);
  const supabase = createClient();
  const { data, error } = await supabase.rpc("get_student_lesson_snapshot", {
    p_room_id: roomId,
  });
