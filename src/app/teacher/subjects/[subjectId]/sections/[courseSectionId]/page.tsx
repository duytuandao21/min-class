...
import { EditLessonButton } from "@/features/lessons/components/edit-lesson-button";
    ...
    <main className="mx-auto min-h-screen w-full max-w-6xl px-6 py-10 sm:px-10">
      ...
      <section className="mb-10" aria-labelledby="lesson-list-title">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <h2 className="text-3xl font-semibold" id="lesson-list-title">Lessons</h2>
            ...
            {detail.chapters.map((chapter, chapterIndex) => {
              ...
              return (
                <LessonChapterDisclosure
                  actions={(
                    <div className="flex flex-wrap items-center justify-end gap-2">
                      ...
                      {chapterLessons.length > 0 ? (
                        activeSession ? (
                          <Link className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-sky-600 px-4 py-2 text-sm font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-sky-700 motion-reduce:transform-none" href={`/teacher/rooms/${activeSession.id}`}>
                            <span aria-hidden className="size-2 animate-pulse rounded-full bg-white motion-reduce:animate-none" />
                            Dashboard
                          </Link>
                        ) : (
                      ...
                      <ChapterOptionsMenu chapterName={chapter.name}>
                          ...
                          {hasSessionHistory ? (
                            <Link
                              className="flex min-h-10 w-full items-center rounded-xl px-3 py-2 text-sm font-bold text-amber-900 transition hover:bg-amber-50"
                  ...
                  subtitle={`${chapterLessons.length} ${chapterLessons.length === 1 ? "Lesson" : "Lessons"}`}
                  title={chapter.name}
                >
                        ...
                        {chapterLessons.map((lesson) => (
                          <li className="flex flex-col gap-4 rounded-xl border border-black/10 bg-white px-4 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-between" key={lesson.id}>
                            ...
                            <div className="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
                              <EditLessonButton
                                href={`/teacher/subjects/${detail.subject.id}/sections/${detail.courseSection.id}/lessons/${lesson.id}/edit`}
                                lessonTitle={lesson.title}
                                locked={lesson.latestSession !== null}
                              />
                              <DeleteLessonButton
                                courseSectionId={detail.courseSection.id}
                                lessonId={lesson.id}
