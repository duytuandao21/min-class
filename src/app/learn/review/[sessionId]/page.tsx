...
async function SessionLessonNavigation({
  selectedLessonId,
  sessionId,
  sessionLessonsPromise,
}: {
  selectedLessonId: string;
  sessionId: string;
  sessionLessonsPromise: ReturnType<typeof getStudentEndedSessionLessons>;
}) {
  const sessionLessons = await sessionLessonsPromise;
  if (sessionLessons.length <= 1) return null;
  return (
    <nav aria-label="Các Lesson trong chương" className="mt-7 flex gap-2 overflow-x-auto pb-3">
      {sessionLessons.map((lesson) => (
        <Link
          aria-current={lesson.lesson_id === selectedLessonId ? "page" : undefined}
          className={`shrink-0 rounded-xl border px-4 py-2.5 text-sm font-bold transition ${lesson.lesson_id === selectedLessonId ? "border-emerald-700 bg-emerald-700 text-white" : "border-black/10 bg-white hover:border-emerald-400 hover:text-[var(--accent)]"}`}
...
export default async function StudentEndedLessonReviewPage({
  ...
  const sessionLessonsPromise = getStudentEndedSessionLessons(sessionId);
    ...
    <main className="mx-auto min-h-screen w-full max-w-6xl px-6 py-10 sm:px-10 sm:py-14 lg:px-12">
      ...
      <header className="mt-10 border-b border-black/10 pb-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <p className="ml-auto inline-flex shrink-0 items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-950">
            <strong className="font-mono text-sm font-bold tracking-wide">{review.mssv}</strong>
          </p>
        </div>
        ...
        <p className="mt-6 max-w-3xl text-lg leading-8 text-[var(--muted)]">
          Bạn có thể xem toàn bộ Section và đáp án Quiz của buổi học đã kết thúc.
        </p>
      </header>

      <Suspense fallback={<div className="mt-7 h-11 w-80 max-w-full animate-pulse rounded-xl bg-black/5 motion-reduce:animate-none" />}>
        <SessionLessonNavigation
          selectedLessonId={review.lessonId}
          sessionId={sessionId}
          sessionLessonsPromise={sessionLessonsPromise}
        />
      </Suspense>
